...
export function scoreCandidate(context: CandidateScoreContext): number {
  ...
  if (generationStyle === 'humanLike') {
    ...
    if (cfMidi !== undefined) {
      const interval = classifyIntervalSemitones(midi - cfMidi, true);
      if (interval === 'imperfect') value += 1.25;
    ...
    if (phraseProgress > 0.8 && midiToPitchClass(midi) === score.tonicPitchClass) value += 2.5;
    if (phraseProgress > 0.7 && midiToPitchClass(midi) === dominantPc) value += 1;
